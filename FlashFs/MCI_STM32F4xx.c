...
static int32_t Initialize (ARM_MCI_SignalEvent_t cb_event) {
  ...
  /* Configure pins */
	/* Enable GPIOD (command), GPIOC (data) clock */
...
/**
  \fn            int32_t AbortTransfer (void)中止数据传输
...
static int32_t Control (uint32_t control, uint32_t arg) {
  ...
  switch (control) {
    case ARM_MCI_BUS_SPEED:			//设置时钟配置总线速度
    ...
    case ARM_MCI_BUS_SPEED_MODE:		//设置总线速度模式
      switch (arg) {
    ...
    case ARM_MCI_BUS_CMD_MODE:				//设置CMD模式
      switch (arg) {
        case ARM_MCI_BUS_CMD_OPEN_DRAIN:	
          /* Configure command line in open-drain mode */
					...
					GPIOD->MODER   &= ~0x0000030;
					GPIOD->MODER   |=  0x0000020;         /* Pins to alternate function         */
					GPIOD->OTYPER  &= ~0x0000004;         /* Configure as open-drain pin         */
					GPIOD->OTYPER  |=  0x0000004;         /* Configure as open-drain pin         */
					GPIOD->PUPDR   &= ~0x0000030;
					GPIOD->PUPDR   |=  0x0000010;         /* Pull-up on pin                     */       
					GPIOD->OSPEEDR |=  0x0000030;         /* Pins output speed to 100MHz        */
    ...
    case ARM_MCI_BUS_DATA_WIDTH:		//设置总线宽度
      switch (arg) {
