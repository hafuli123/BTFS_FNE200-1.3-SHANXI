 ...
 *              (2)  EE_Init()                  //初始化flash
 *              (3)  EE_WriteVariable(uint16_t VirtAddress, uint16_t len, void *Data)   //写操作
 *                   EE_ReadVariable(uint16_t VirtAddress, uint16_t len, void *Data)   //读操作
            ...
            if (PageStatus1 == VALID_PAGE) /* Page0 receive, Page1 valid */
            {
                ...
                for (VarIdx = 0; VarIdx < NUM_OF_VAL_CELL; VarIdx++)
                {
                    if (VarIdx != x0)
                    {
                        ...
                        ReadStatus = EE_ReadVariable(VarIdx, EEPROM_CELL_DATA_LEN, DatWorkBuf);
...
static uint16_t EE_VerifyPageFullWriteVariable(uint16_t VirtAddress, void *data, uint16_t len)
{
    ...
    while (Address < PageEndAddress)
    {
            ...
            && ((*(__IO uint32_t*)(Address + EEPROM_CELL_SZ-4)) == 0xFFFFFFFF))
        {
            ...
            {
                FlashStatus = HAL_FLASH_Program(TYPEPROGRAM_HALFWORD, Address + i * 2, Data[i]);
                /* If program operation was failed, a Flash error code is returned */
                if (FlashStatus != FLASH_COMPLETE)
                {
                    return FlashStatus;
            ...
            FlashStatus = HAL_FLASH_Program(TYPEPROGRAM_HALFWORD, Address + EEPROM_CELL_DATA_LEN, VirtAddress);
...
static uint16_t EE_PageTransfer(uint16_t VirtAddress, void *Data, uint16_t len)
{
    ...
    for (VarIdx = 0; VarIdx < NUM_OF_VAL_CELL; VarIdx++)
    {
        if (VarIdx != VirtAddress)  /* Check each variable except the one passed as parameter */
        {
            ...
            ReadStatus = EE_ReadVariable(VarIdx, len, DatWorkBuf);
            /* In case variable corresponding to the virtual address was found */
