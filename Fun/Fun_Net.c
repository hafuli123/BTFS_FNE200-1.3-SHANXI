...
static uint8_t http_Post_File_Start_Sta = 0;
//HTTP上传文件启动
uint8_t Fun_Gprs_Http_Post_File_Start(char *url,char *username,char *password,char*fileName,uint32_t fileSize)
{
	//已经启动
	if(http_Post_File_Start_Sta == 1)
	{
		http_Post_File_Start_Sta = 0;
		osMutexRelease(mutexId);
		return 0;
	}
	osMutexAcquire(mutexId,osWaitForever);
	uint32_t ret = Chip_Ec20Http_Post_File_Start(url,username,password,fileName,fileSize);
	//启动失败
	if(ret == 0)
	{
		http_Post_File_Start_Sta = 0;
...
{
	//未启动,直接返回
	if(http_Post_File_Start_Sta == 0)
	{
		return 0;
	}
	uint32_t ret = Chip_Ec20Http_Post_File_Input(buff,len);
	if(ret == 0)
	{
		http_Post_File_Start_Sta = 0;
		osMutexRelease(mutexId);
