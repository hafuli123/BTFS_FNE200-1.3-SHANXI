...
#include "cmsis_os2.h"
#include "bsp_rtc.h"
const char* carType = "SDHN_FV2.02";
		...
		case 0x18FFABF3:	 
			{
				...
				//单体总数
				n16Val= calcRealValue(48,16,1,0,CALC_MODE,msg->data,HIGHTLOW_MODE,CAN_MODE);
				if(n16Val <= MAX_VOL_CNT)
					gRealData.subSysData[0].singleVolCnt = n16Val;
				//温度探针总数
				n8Val = calcRealValue(32,8,1,0,CALC_MODE,msg->data,HIGHTLOW_MODE,CAN_MODE);	
				if(n8Val <= MAX_TMP_CNT)
