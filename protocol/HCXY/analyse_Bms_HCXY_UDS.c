...
{
	if(gRealData.carState == 0 || recvCanCnt < 2000) {
		return IN_TESTING;
	}
...
{
	if(gRealData.carState == 0 || recvCanCnt < 2000 || osKernelGetTickCount() - slowChargeTime >= 1000) {
		return IN_TESTING;
	}
...
void udsProc(void)
{
	...
	gprsSignal[0] = 0;
	gprsSignal[1] = gTerminalState.csq;
  ...
  SnapshotBcdTime[0] = 0x20;
	...
	Diagnostic_Proc();
	if(gLogoutCmd == 1 && gRealData.carState == 0)
	{
		if(UnintUds == 0)
		{
			UnintUds = 1;
...
void udsInit(uint8_t ch)
{
	...
	memcpy(ecuId,gFrimPara.yztDevId,strlen(gFrimPara.yztDevId));
	memcpy(vin1,gSysPara.vinCode,17);
	memcpy(vin2,gSysPara.vinCode,17);
	Diagnostic_Init(0x682, 0x602 , 0x7DF, 0 , 1024 , SendFrame,50,2000);
	//********************************** service 10*****************************************//
	...
	InitAddDID(0xE001, 20, gTerminalState.iccid ,  REALTIME_DID , NULL , READONLY , 0 ,TRUE);							//ICCID
	InitAddDID(0xE002, 15, gTerminalState.imei ,  REALTIME_DID , NULL , READONLY , 0 ,TRUE);							//IMEI
	#if 1
	...
	Diagnostic_SetNLParam(70, 150, 150, 70, 20, 70, 0, 20, 0);
	udsCanch = ch;
}
...
void udsRecvData(uint8_t ch,CAN_msg *msg)
{
	...
	{
		Diagnostic_RxFrame(msg->id, msg->data , msg->format , msg->len , msg->type);	
	}
}
