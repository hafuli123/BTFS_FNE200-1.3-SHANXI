...
static void saveCanData(uint8_t ch,uint32_t canId,uint8_t *canData);				//存储实时CAN报文到本地
...
static const osMutexAttr_t saveCANMutex = { NULL, osMutexRecursive|osMutexPrioInherit, NULL, 0 };//EMMC使用互斥信号，防止多线程同时调用
static osMutexId_t saveCANMutexId;

#define CAN_LOG_INTERVAL 	0					//CAN日志间隔
#define SDFREE_SPACE (120*1024*1024)			//SD容量最小 空间 120M
#define FDELETTE_INTERVAL	(1000*60*10)		//删除文件执行时间间隔
#define SDCARD_MAX_FILE_CNT 500 				//SD卡文件最大个数
static uint32_t canlogSaveStamp = 0;			//CAN日志记录周期
char filePath[40] = {0};						//文件路径
static char canlogFileName[40] = {0};			//CAN日志文件名
static uint32_t canlogTimeStamp = 0;			//CAN日志起始时间戳
static uint32_t canlogflushTimeStamp = 0;		//CAN日志刷新时间戳
static FILE* canlogFileHandle = NULL;			//CAN日志文件句柄
static uint32_t timmms = 0;						//存储时间间隔
static uint32_t checkFileTime = 0;				//删除文件执行时间标志

uint8_t printfBuf[8096] = {0};					//写数据缓冲区
uint8_t isCreatMute = 0;						//重启初始互斥信号

//SD卡存满 删除旧文件
static void fileCycleDelete(void)
{
	uint32_t fileCnt = 0;
	char maxDateFileName[40] = {0};		//用于校时，防止终端时间不准
	char minDateFileName[40] = {0};		//用于删除，防止SD空间不够
	gTerminalState.sdFreeCap = ffree("M0:");
	
	static fsFileInfo info;
	//遍历文件
	while (ffind ("M0:\\RealCANData\\*.*",&info) == 0)
	{
		fileCnt++;
		//异常文件名
		if(strspn((char*)info.name,"0123456789") < 8)
		{
			if(fileCnt >= SDCARD_MAX_FILE_CNT || gTerminalState.sdFreeCap < SDFREE_SPACE)
			{
				fdelete((char*)info.name,NULL);
			}
		}
		else
		{
			if(strlen(maxDateFileName) == 0 || memcmp(maxDateFileName,info.name,8) < 0)
			{
				strcpy(maxDateFileName,(char*)info.name);
			}
			if(strlen(minDateFileName) == 0 || memcmp(minDateFileName,info.name,8) > 0)
			{
				strcpy(minDateFileName,(char*)info.name);
			}
		}
	}
	//数量或剩余空间超限，删除老文件
	if((fileCnt >= SDCARD_MAX_FILE_CNT  || gTerminalState.sdFreeCap < SDFREE_SPACE) && strlen(minDateFileName) > 0)
	{
		fdelete(minDateFileName,NULL);
	}
	//校验时间
	memset(minDateFileName,0,sizeof(minDateFileName));
	sscanf(maxDateFileName,"%*02d%02hhu%02hhu%02hhu",&minDateFileName[0],&minDateFileName[1],&minDateFileName[2]);
	memset(maxDateFileName,0,sizeof(maxDateFileName));
	maxDateFileName[0] = g_system_dt.year - 2000;maxDateFileName[1] = g_system_dt.month;maxDateFileName[2] = g_system_dt.day;
	//终端时间复位
	if(memcmp(maxDateFileName,minDateFileName,3) < 0)
	{
			g_system_dt.year = 2000 + minDateFileName[0];
			g_system_dt.month = minDateFileName[1];
			g_system_dt.day = minDateFileName[2];
			g_system_dt.hour = g_system_dt.hour % 24;
			g_system_dt.minute = g_system_dt.minute % 60;
			g_system_dt.second = g_system_dt.second % 60;
			RTC_Time_Set(&g_system_dt);		
	}
	//重新获取剩余空间
	gTerminalState.sdFreeCap =  ffree("M0:");
}
...
static void saveCanData(uint8_t ch,uint32_t canId,uint8_t *canData)
{
	...
	if(isCreatMute == 0)
	{
		isCreatMute = 1;
		//创建互斥信号量 
		saveCANMutexId = osMutexNew (&saveCANMutex);
		gTerminalState.sdFreeCap = ffree("M0:");
		checkFileTime = osKernelGetTickCount();
	}
	osMutexAcquire(saveCANMutexId,osWaitForever);	
	//剩余容量大于120M
	if(gTerminalState.sdFreeCap > SDFREE_SPACE && (osKernelGetTickCount() - canlogSaveStamp) >= CAN_LOG_INTERVAL)
	{
		canlogSaveStamp = osKernelGetTickCount();
		sprintf(filePath,"M0:\\RealCANData\\%04d%02d%02d%02d%02d%02d.asc",g_system_dt.year,g_system_dt.month,g_system_dt.day,g_system_dt.hour,g_system_dt.minute,g_system_dt.second);
		//文件命名精确到分钟，1小时一个文件
		if(strncmp(filePath,canlogFileName,10) != 0 || canlogFileHandle == NULL)
		{
			strcpy(canlogFileName,filePath);
			//时钟进位，保存旧文件，新建文件
			if(canlogFileHandle != NULL)
			{
				fflush(canlogFileHandle);
				fclose(canlogFileHandle);
				canlogFileHandle = NULL;
			}
			canlogFileHandle = fopen(canlogFileName,"w");
			if(canlogFileHandle != NULL)
			{
				memset(printfBuf,0,sizeof(printfBuf));
				canlogTimeStamp =  osKernelGetTickCount();
				//date Wed Apr 16 09:21:13.159 am 2014
				wbLen = fprintf(canlogFileHandle,"FirstFrame Time %04d-%02d-%02d %02d:%02d:%02d\r\n",g_system_dt.year,g_system_dt.month,g_system_dt.day,g_system_dt.hour,g_system_dt.minute,g_system_dt.second);
				wbLen = fprintf(canlogFileHandle,"base hex timestamps absolute\r\n");
				fflush(canlogFileHandle);
				if(wbLen <= 0)
				{
					//新文件写入失败
		...
		{
			uint16_t wLens = 0;
			timeStamp = osKernelGetTickCount() - canlogTimeStamp;
			wbLen = strlen((char*)printfBuf);
			sprintf((char*)&printfBuf[wbLen],"%d.%03d %d %xx Rx d 8 %02x %02x %02x %02x %02x %02x %02x %02x\r\n",timeStamp/1000,timeStamp%1000,ch + 1,canId,canData[0],canData[1],canData[2],canData[3],canData[4],canData[5],canData[6],canData[7]);
			if(wbLen + 120 > sizeof(printfBuf))
			{
				wLens = fwrite((char*)printfBuf,strlen((char*)printfBuf),1,canlogFileHandle);
				memset(printfBuf,0,sizeof(printfBuf));
				if(wLens == 1)
				{
					fflush(canlogFileHandle);
			...
			timmms = osKernelGetTickCount() - canlogflushTimeStamp;
			if(timmms >= 1000 )
			{
				//无CAN或10秒保存一次，提高存储效率
				canlogflushTimeStamp = osKernelGetTickCount();
				fclose(canlogFileHandle);
				canlogFileHandle = NULL;
				canlogFileHandle = fopen(canlogFileName,"ab+");
			}
		}
	}
	osMutexRelease(saveCANMutexId);
	
	//SD卡存满 删除旧文件
	if(osKernelGetTickCount() - checkFileTime> FDELETTE_INTERVAL)			//3分钟执行一次文件查重
	{
		checkFileTime = osKernelGetTickCount();	
