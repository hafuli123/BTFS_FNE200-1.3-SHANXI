...
static const char* carType = "F4_SJHT_FV_T1.01";
...
static uint8_t actAreaInjectValHz = 0;
static uint8_t totalUreaUsedHz = 0;
static uint8_t engReferenceTorqueHz = 0;
...
void unpackFvCAN(uint8_t ch,CAN_msg *msg)
{
	...
	uint8_t sourseAddr = (msg->id & 0x000000FF) >> 0;
...
//			break;				
	...
	if(sendStaSecond != g_system_dt.second)
	{
		uint8_t get1939 = 0;
		CAN_msg msg_buf = {0x18FA122B,{ 0x00, 0x00, 0x00, 0x00,0x00, 0x00, 0x00, 0x00 },8,0,EXTENDED_TYPE,DATA_FRAME};

		sendStaSecond = g_system_dt.second;	
		//if(((gSysPara.linkSwitch >> SUP_DATA_BIT) & 0x03) > 0)
		{
			if(actAreaInjectValHz != 0xFF)
			{
				actAreaInjectValHz++;
				if(actAreaInjectValHz > 5 && get1939 == 0)
				{
					get1939 = 1;
					actAreaInjectValHz = 0;
			...
			if(totalUreaUsedHz != 0xFF)
			{
				totalUreaUsedHz++;
				if(totalUreaUsedHz > 5 && get1939 == 0)
				{
					get1939 = 1;
					totalUreaUsedHz = 0;
			...
			if(engReferenceTorqueHz != 0xFF)
			{
				engReferenceTorqueHz++;
				if(engReferenceTorqueHz > 5 && get1939 == 0)
				{
					get1939 = 1;
					engReferenceTorqueHz = 0;
	...
	if(msg->id == 0x18daf100 || msg->id == 0x18daf110 || msg->id == 0x7E8)
	{
		...
		if(udsrecvDataFLag == 0)//接收数据完成
		{
			...
			else if((recvUdsData[0]==0x62 && recvUdsData[1]== 0xF4 && recvUdsData[2]== 0x01) || (recvUdsData[0] == 0x41 && recvUdsData[1]== 0x01))
			{
				...
				//NMHC氧化催化器
				gRealData.diagRdyState.nmhcConvertingCatalyst = calcCanValue(0,1,&D,HIGHTLOW_MODE,CAN_MODE);
				//选择性催化还原系统（SCR）或NOx吸附器
				gRealData.diagRdyState.scrOrNOxAdsorber = calcCanValue(1,1,&D,HIGHTLOW_MODE,CAN_MODE);				
				//增压压力控制系统
				gRealData.diagRdyState.boostPressureCtrlSys = calcCanValue(3,1,&D,HIGHTLOW_MODE,CAN_MODE);
				//AC系统制冷剂
				...
				//DPF监控
				gRealData.diagRdyState.dpf = calcCanValue(6,1,&D,HIGHTLOW_MODE,CAN_MODE);
				//EGR和Vvt系统
				...
				//排气传感器加热器
				//gRealData.diagSpState.exhaustGasSensorHeater = 0;
				//冷启动辅助系统
				//gRealData.diagSpState.coldStartAidSys = 0;
				
				//排气传感器加热器
				//gRealData.diagRdyState.exhaustGasSensorHeater = 0;
				//冷启动辅助系统
