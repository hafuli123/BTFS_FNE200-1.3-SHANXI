...
uint8_t autoChgSW = 0;				//0关闭自动补电，1开启自动补电
...
enum PARA_ID{
	...
	PARA_GETSENDVINSIGN = 0xA6,				//VIN变更上送标志
};
...
uint8_t allow_FTPUpdata = 0;								//允许FTP升级
...
/* 充电电量参数 */
RTC_INFOR chgStartTimes;										//充电起始时间 
RTC_INFOR chgEndTimes;											//充电结束时间
static uint8_t chgStaST = 0;								//充电起始状态位
static float chgPower = 0;									//累计充电电量
static uint32_t chgStartTime = 0;						//充电计算间隔
static uint8_t chgPowerSign = 0;						//充电量上送标志 
static uint32_t reportChgPInterval = 10000;				//默认上送间隔是10s
uint8_t reportChgPfaildCnt = 0;										//上送VIN失败次数
uint8_t culcalTime = 0;

/* 自动补电参数 */
uint8_t sAllowAutoChg	= 0;									//被动 补电指令许可位
uint8_t autoChgState = 0;										//主动 补电状态位：0 未开启，1  开启

/* 定位异常 */
static uint8_t isLocFault = 0;

static void AutoVoltInit();									//补电，欠压，低电默认值初始化							
static void lowVoltProtect(void);						//终端低电保护
static void autoChgPower(void);							//自动补电逻辑
static void localtionFaultFun(void);				//定位异常判断

static uint16_t Pack0AData(RealData *pRealData,uint8_t *bPackBuf);														//打包自定义数据
static uint16_t packCHGPower(uint8_t link,char* vin,uint8_t* buff,uint16_t maxLen);						//上送充电电量
static uint16_t packUpdataVIN(uint8_t link,char* vin,uint8_t* buff,uint16_t maxLen);					//首次获取VIN 上报

static uint8_t UnpackLockCMD(uint8_t link,char* vin,const uint8_t *rbuf, uint16_t rLen);				//锁车指令
static uint8_t UnpackSoftVer_Change(uint8_t link,char* vin,const uint8_t *rbuf, uint16_t rLen);	//锁车版本
static uint8_t UnpackBindingCtl(uint8_t link,char* vin,const uint8_t *rbuf, uint16_t rLen);			//终端绑定
static uint8_t UnpackDoorCtl(uint8_t link,char* vin,const uint8_t *rbuf, uint16_t rLen);				//车门控制
static uint8_t UnpackMaintTips(uint8_t link,char* vin,const uint8_t *rbuf, uint16_t rLen);			//保养提示
static uint8_t UnpackPayTips(uint8_t link,char* vin,const uint8_t *rbuf, uint16_t rLen);				//缴费提示
static uint8_t UnpackYearCheckTips(uint8_t link,char* vin,const uint8_t *rbuf, uint16_t rLen);	//年审提示
...
static void wk_lineTime(void)
{
	...
	{		
		...
		if(osKernelGetTickCount() - wk_timesss > (gUserDara.wk_SendInterval-3)*10000)
		{
			allow_FTPUpdata = 1;
		}
...
//			if(getFTPModeState() == 1)																													/* 含有远程升级指令，10min后启动升级 */
//			{
//				bsp_wakelock_para(wakelock,wakeTime,600,wakeCallback,0,0);												/* 10分钟 */
//			}
//			else
//			{
...
void* extInit(uint8_t link,char* vin,uint8_t prot,uint8_t* buff,uint16_t buffLen)
{
	...
	//自定义数据分配内存，设置偏移量
	p0Aoffset = 0;
	pSelfData0A = (SelfData0A*)&gRealData.externData[p0Aoffset];
	
	wakelock = bsp_wakelock_create();			/* 创建唤醒锁 */	
	/*setWakeTime();												 设置唤醒时间 */
	
	AutoVoltInit();												/* 自动补电,低电保护初始化 */
...
uint8_t extRun(void* obj,uint8_t ctrl,uint8_t sta)
{
	...
	if(pSta->bLink == 0)
	{
		...
		if(sta == 1)
		{
			...
			else if(gUserDara.fisrtGetVIN == 1 && osKernelGetTickCount() - reportVINStamp >= reportVINInterval)	
			{
		...
		case CMD_SENDVINCMD:																						//首次获取VIN上送响应
			{
				if(szRecvBuf[3] == 0x01)
				{
					gUserDara.fisrtGetVIN	 = 0;																//响应成功后不进行上报
					saveUserData();																						//保存VIN变更标志
				}			
			}
			break;
		default: 
...
uint8_t extReadPara(uint8_t readParaId,uint8_t* readBuff)
{
	...
	switch(readParaId)
	{
		...
		case PARA_GETCANLOG:
			...
			break;
		case PARA_GETSENDVINSIGN:
			readBuff[readlen++] = (gUserDara.fisrtGetVIN == 1? 0:1);
			break;
	}	
	return readlen;
}
...
static uint16_t Pack0AData(RealData *pRealData,uint8_t *bPackBuf)
{
	...
	memcpy(&bPackBuf[index],p0AData->sVCUVerNum,8);												//VCU版本号 p0AData->sVCUVerNum	
	index += 8;
...
static uint8_t UnpackMaintTips(uint8_t link,char* vin,const uint8_t *rbuf, uint16_t rLen)
{
	...
	gUserDara.maintCMD = rbuf[24];										//保养提示
...
static uint8_t UnpackYearCheckTips(uint8_t link,char* vin,const uint8_t *rbuf, uint16_t rLen)
{
	...
	gUserDara.yearCheckCMD = rbuf[24];								//年审提示														
