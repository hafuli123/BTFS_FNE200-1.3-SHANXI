...
typedef struct{
	...
	uint8_t isSiteExcute;				//无效定位时间标志
	uint8_t outlineCMD;					//网络限速指令
	
	uint8_t fisrtGetVIN;				//是否是初次获取到正确的VIN  0：未传成功 1：传成功
...
extern void calculCHGPower(void);																															//充电量计算
extern uint8_t isChgVIN;																//VIN 切换标志开启
