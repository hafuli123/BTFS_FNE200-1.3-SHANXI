...
#define JIANHAO 1							//0：BC015；  1：BC043；  2：BC023;  3:BC013;

static char carType[30] = {0};
//static const char* carType = "F4_HCXYDST_STD103_TEST2";
...
void iniEvCanData(void)
{
	if(bsp_first_start() == 1 || gSysPara.store_flag != 0xAA)
	{
		...
		gSysPara.linkSwitch = 0x03;															//双链路标准
		...
		if(JIANHAO == 0)				//BC015
		{
			strcpy(carType,"F4_XYDST103_BC015_V1.02");									//车型
			strcpy(gSysPara.domain[1],"zijian.shineraynewenergy.com");
			gSysPara.port[1] = 19006;	
		}
		else if(JIANHAO == 1)		//BC043
		{
			strcpy(carType,"F4_XYDST103_BC043_V1.02");									//车型
			memcpy(gSysPara.domain[1],"101.227.76.140",14);
			gSysPara.port[1] = 19006;		
		}
		else if(JIANHAO == 2)		//BC023
		{	
			strcpy(carType,"F4_XYDST103_BC023_V1.02");									//车型
			memcpy(gSysPara.domain[1],"data.wanxiangauto.com",21);
			gSysPara.port[1] = 19006;		
		}
		else if(JIANHAO == 3)		//BC013
		{	
			strcpy(carType,"F4_XYDST103_BC013_V1.02");									//车型
			gSysPara.linkSwitch = 0x01;				//单连地上铁链路
		}
	...
	{
		strcpy(carType,"F4_XYDST103_BC043_V1.02");
	}
	else if(JIANHAO == 2)		//BC023
	{	
		strcpy(carType,"F4_XYDST103_BC023_V1.02");
	}
	else if(JIANHAO == 3)		//BC013
	{	
		strcpy(carType,"F4_XYDST103_BC013_V1.02");
	}
	...
	CAR_TYPE = (const char*)&carType;
	gRealData.rechargeSysCodeLen = 0;
	
	memcpy(recordVin,gSysPara.vinCode,sizeof(gSysPara.vinCode));
	//udsInit(1);
}
...
void unpackEvCAN(uint8_t ch,CAN_msg *msg)
{
	...
	switch(msg->id)
	{
		...
		case 0x237:
		{
			...
			if(isChgVIN == 0 && (memcmp(realVin,gSysPara.vinCode,17) != 0 || memcmp(recordVin,gSysPara.vinCode,17) != 0)&& strlen(realVin) == 17 && CheckVin(realVin))
			{
				if(!CheckVin(gSysPara.vinCode))
				{
				...
				memcpy(recordVin,gSysPara.vinCode,sizeof(gSysPara.vinCode));
				gUserDara.fisrtGetVIN = 1;																	//保存VIN变更标志
				saveUserData();
				isChgVIN = 1;																								//检测到VIN变更
			}
		}
