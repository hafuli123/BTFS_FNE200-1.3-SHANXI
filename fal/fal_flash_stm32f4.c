...
static int write(long offset, const uint8_t *buf, size_t size)
{
    ...
    {
        /* write data to flash */
        if (FLASH_ProgramByte(addr, (uint64_t)(*buf)) == FLASH_COMPLETE)
        {
            if (*(uint8_t *) addr != *buf)
            {
                return -1;
...
{
    uint32_t FirstSector = 0, LastSectors = 0;
    ...
    LastSectors = stm32_get_sector(addr + size - 1);

		while(FirstSector <= LastSectors)
		{
