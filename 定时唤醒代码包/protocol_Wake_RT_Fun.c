...
#include "./protocol_Wake_RT_Fun.h"
...
static void wake_RT_SetTime(uint8_t swID)							
{
	...
	{		
		...
		if(swID == 1)				//开启定时唤醒
		{
			...
			{
				bsp_wakelock_para(wakelock,wakeTime,gUserDara.wk_Interval,wake_RT_Callback,0,0);			/* 默认6小时 */
			}
		}
		else 								//关闭定时唤醒
		{
...
{	
		if(getFTPModeState() == 0)					//无升级指令，关闭定时唤醒 进入硬件休眠		
		{
			wake_RT_SetTime(0);															
		}
		else
		{
			wake_RT_SetTime(1);								//有升级指令，恢复定时唤醒 进入软件休眠
		}
