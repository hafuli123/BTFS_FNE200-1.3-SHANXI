...
int tms_sha256_update(unsigned char *data, unsigned int data_len)
{
    int rv = SUCCEED;
    TRANSMIT_DATA apdu;
    unsigned char buf[APDU_BUFFER_MAX_SIZE] = {0};
...
int tms_sha256_final(unsigned char *data, unsigned int data_len, unsigned char *hash, unsigned int *hash_len)
{
    int rv = SUCCEED;
    TRANSMIT_DATA apdu;
    unsigned char buf[APDU_BUFFER_MAX_SIZE] = {0};
    ...
    if (hash == NULL) {//返回实际响应长度
        *hash_len = SHA256_HASH_SIZE;
        ...
        if (*hash_len < SHA256_HASH_SIZE) {
            printf("[%s] %d :hash_len(%d) is less than %d bytes. \n", __func__, __LINE__, *hash_len, SHA256_HASH_SIZE);
            rv = FAILED;
            goto end;
        }
    }
